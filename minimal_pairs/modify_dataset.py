...
from utils.impossible_utils import PERTURBATIONS_PAIR, PERTURBATION_TO_HF_MODEL_NAME
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Modify a BLiMP dataset into an impossible dataset using a specified perturbation."
    )
    parser.add_argument(
        "base_dataset_path", type=str, help="Path to the base BLiMP dataset (.jsonl)"
    )
    parser.add_argument(
        "impossible_language_option",
        type=str,
        help="Perturbation option (must be a key in PERTURBATION_TO_HF_MODEL_NAME)",
    )
    parser.add_argument(
        "--output_path",
        type=str,
        default=None,
        help="Optional output path. Defaults to input path + '%' + perturbation id + extension.",
    )
...
def modify_dataset(base_dataset_path, impossible_language_option, output_path=None):
    if impossible_language_option not in PERTURBATION_TO_HF_MODEL_NAME:
        raise ValueError(
            f"Invalid impossible_language_option '{impossible_language_option}'. Must be one of: {list(PERTURBATION_TO_HF_MODEL_NAME.keys())}"
        )
    ...
    if output_path is None:
        output_path = os.path.join(
            base_dir, f"{fname}%{impossible_language_option}{ext}"
        )
    ...
    try:
        with open(base_dataset_path, "r") as infile, open(output_path, "w") as outfile:
            ...
            for line in tqdm(infile, desc="Processing", unit=" sentences"):
                ...
                impossible_good = "".join(
                    map(lambda x: tokenizer.decode(x), impossible_good)
                )
                impossible_bad = "".join(
                    map(lambda x: tokenizer.decode(x), impossible_bad)
                )
...
def main():
    ...
    return modify_dataset(
        args.base_dataset_path, args.impossible_language_option, args.output_path
    )
