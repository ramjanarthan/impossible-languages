...
from minimal_pairs.evaluation_dataset import DataBatchLoader
from minimal_pairs.results import DEFAULT_MODEL_CHECKPOINT, append_result
from utils.impossible_utils import PERTURBATION_TO_HF_MODEL_NAME
...
class Evaluator:
    ...
    def __init__(
        ...
        self.model, self.tokenizer = self._load_model_and_tokenizer(
            self.hugging_face_model_name, self.device, self.checkpoint
        )

        # Parse dataset information from filename
        self.grammatical_phenomenon = Path(self.dataset_path).stem
        self.dataset_language = model_name
    ...
    def evaluate(self):
        ...
        dataset_base = (
            os.path.basename(self.dataset_path).replace(".jsonl", "").replace("%", "_")
        )
        raw_rows = []
        ...
        for batch in batch_loader:
            ...
            logprobs_good = get_sentence_log_probabilities(
                self.model, self.tokenizer, sentences_good, device=self.device
            )
            logprobs_bad = get_sentence_log_probabilities(
                self.model, self.tokenizer, sentences_bad, device=self.device
            )

            correct_predictions += sum(
                g > b for g, b in zip(logprobs_good, logprobs_bad)
            )
            total_sentences += len(sentences_good)

            # 2. Calculate per-sentence perplexities
            perplexities_good = get_perplexities(
                self.model, self.tokenizer, sentences_good, device=self.device
            )
            perplexities_bad = get_perplexities(
                self.model, self.tokenizer, sentences_bad, device=self.device
            )
            ...
            for good_s, good_lp, good_ppl, bad_s, bad_lp, bad_ppl in zip(
                ...
                raw_rows.append(
                    [good_s, good_lp, good_ppl, bad_s, bad_lp, bad_ppl, correct]
                )
        ...
        geo_mean_perplexity_good = calculate_geometric_mean_perplexity(
            all_perplexities_good
        )
        geo_mean_perplexity_bad = calculate_geometric_mean_perplexity(
            all_perplexities_bad
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Evaluate a model on a dataset for a grammatical phenomenon."
    )
    parser.add_argument(
        "--dataset", type=str, required=True, help="Path to the dataset JSONL file."
    )
    parser.add_argument(
        "--model_name",
        type=str,
        required=True,
        help="Hugging Face model name (e.g., 'gpt2').",
    )
    parser.add_argument(
