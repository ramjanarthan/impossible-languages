...
def calculate_geometric_mean_perplexity(perplexities):
    ...
    valid_perps = [
        p for p in perplexities if p > 0 and not np.isnan(p) and not np.isinf(p)
    ]
    if not valid_perps:
        return float("inf")
...
def get_perplexities(
    model, tokenizer, sentences: List[str], device="mps", max_length=512
):
...
def get_perplexities_from_token_ids(
    model, tokenizer, token_id_lists: List[List[int]], device="mps"
):
    ...
    with torch.no_grad():
        ...
        average_losses[valid_token_mask] = (
            sentence_losses[valid_token_mask] / valid_token_counts[valid_token_mask]
        )
...
def get_sentence_log_probabilities(
    model, tokenizer, sentences: List[str], device="mps", max_length=512
):
    """
