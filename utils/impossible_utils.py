# utils.py
# Original Author: Julie Kallini
# Modified by: Coleman Haley and Ram Janarthan
...
gpt2_hop_tokenizer = get_gpt2_tokenizer_with_markers([MARKER_HOP_SING, MARKER_HOP_PLUR])
# Get ids of marker tokens
marker_sg_token = gpt2_hop_tokenizer.get_added_vocab()[MARKER_HOP_SING]
marker_pl_token = gpt2_hop_tokenizer.get_added_vocab()[MARKER_HOP_PLUR]


# GPT-2 reverse tokenization
gpt2_rev_tokenizer = get_gpt2_tokenizer_with_markers([MARKER_REV])
# Get ids of marker tokens
marker_rev_token = gpt2_rev_tokenizer.get_added_vocab()[MARKER_REV]

# GPT-2 determiner tokenization
gpt2_det_tokenizer = get_gpt2_tokenizer_with_markers([BOS_TOKEN])
# Get id of BOS token
...
def compute_surprisals(model, input_ids):
    ...
    with torch.no_grad():
        ...
        true_log_probs = log_probs.gather(2, shifted_input_ids.unsqueeze(-1)).squeeze(
            -1
        )
...
def compute_token_probabilities(model, input_ids, token_id, pad_token_id):
    ...
    for batch_i, probs in enumerate(token_probs):
        ...
        filtered = [
            p if input_ids_seq[pos_i + 1] != pad_token_id else None
            for pos_i, p in enumerate(probs.tolist())
        ]
    ...
    for word in word_annotations:
        ...
        if __affect_hop_word(word):
            ...
            while skipped_words < num_hops and insert_index > 0:
                ...
                if not any([c.isalnum() for c in "".join(new_sent[:insert_index])]):
                    break
            ...
            if any([c.isalnum() for c in "".join(new_sent[:insert_index])]):
                while insert_index != 0 and (
                    new_sent[insert_index] in PART_TOKENS
            ...
            if "Number=Sing" in word["feats"]:
                ...
                raise Exception("Number not in verb features\n" + sent["sent_text"])
...
def affect_hop(sent):
    return (
        any([__affect_hop_word(word) for word in sent["word_annotations"]])
...
PERTURBATIONS = {
    "shuffle_control": {
        "perturbation_function": partial(
            perturb_shuffle_deterministic, seed=None, shuffle=False
        ),
    ...
    "shuffle_nondeterministic": {
        "perturbation_function": partial(
            perturb_shuffle_nondeterministic, rng=default_rng(0)
        ),
        "affect_function": affect_shuffle,
        "filter_function": filter_shuffle,
        "gpt2_tokenizer": gpt2_original_tokenizer,
    ...
    "shuffle_deterministic21": {
        "perturbation_function": partial(
            perturb_shuffle_deterministic, seed=21, shuffle=True
        ),
        "affect_function": affect_shuffle,
        "filter_function": filter_shuffle,
        "gpt2_tokenizer": gpt2_original_tokenizer,
    ...
    "shuffle_deterministic57": {
        "perturbation_function": partial(
            perturb_shuffle_deterministic, seed=57, shuffle=True
        ),
        "affect_function": affect_shuffle,
        "filter_function": filter_shuffle,
        "gpt2_tokenizer": gpt2_original_tokenizer,
    ...
    "shuffle_deterministic84": {
        "perturbation_function": partial(
            perturb_shuffle_deterministic, seed=84, shuffle=True
        ),
        "affect_function": affect_shuffle,
        "filter_function": filter_shuffle,
        "gpt2_tokenizer": gpt2_original_tokenizer,
    ...
    "reverse_control": {
        "perturbation_function": partial(
            perturb_reverse, rng=default_rng(21), reverse=False, full=False
        ),
    ...
    "reverse_partial": {
        "perturbation_function": partial(
            perturb_reverse, rng=default_rng(21), reverse=True, full=False
        ),
        "affect_function": affect_reverse,
        "filter_function": filter_reverse,
        "gpt2_tokenizer": gpt2_rev_tokenizer,
    ...
    "reverse_full": {
        "perturbation_function": partial(
            perturb_reverse, rng=default_rng(21), reverse=False, full=True
        ),
        "affect_function": affect_reverse,
        "filter_function": filter_reverse,
        "gpt2_tokenizer": gpt2_rev_tokenizer,
...
UNDO_PERTURBATIONS = {
    ...
    "shuffle_deterministic21": {
        "perturbation_function": partial(
            undo_perturb_shuffle_deterministic, seed=21, shuffle=True
        ),
        "affect_function": affect_shuffle,
        "filter_function": filter_shuffle,
        "gpt2_tokenizer": gpt2_original_tokenizer,
    ...
    "reverse_partial": {
        "perturbation_function": partial(
            undo_perturb_reverse, reverse=True, full=False
        ),
        "affect_function": affect_reverse,
        "filter_function": filter_reverse,
        "gpt2_tokenizer": gpt2_rev_tokenizer,
    ...
    "reverse_full": {
        "perturbation_function": partial(
            undo_perturb_reverse, reverse=False, full=True
        ),
        "affect_function": affect_reverse,
        "filter_function": filter_reverse,
        "gpt2_tokenizer": gpt2_rev_tokenizer,
...
PERTURBATIONS_PAIR = {
    "shuffle_nondeterministic": {
        "perturbation_function": partial(
            perturb_shuffle_nondeterministic_pair, rng=default_rng(0)
        ),
        "affect_function": affect_shuffle,
        "filter_function": filter_shuffle,
        "gpt2_tokenizer": gpt2_original_tokenizer,
    ...
    "shuffle_deterministic21": {
        "perturbation_function": partial(
            perturb_shuffle_deterministic_pair, seed=21, shuffle=True
        ),
        "affect_function": affect_shuffle,
        "filter_function": filter_shuffle,
        "gpt2_tokenizer": gpt2_original_tokenizer,
    ...
    "shuffle_deterministic57": {
        "perturbation_function": partial(
            perturb_shuffle_deterministic_pair, seed=57, shuffle=True
        ),
        "affect_function": affect_shuffle,
        "filter_function": filter_shuffle,
        "gpt2_tokenizer": gpt2_original_tokenizer,
    ...
    "shuffle_deterministic84": {
        "perturbation_function": partial(
            perturb_shuffle_deterministic_pair, seed=84, shuffle=True
        ),
        "affect_function": affect_shuffle,
        "filter_function": filter_shuffle,
        "gpt2_tokenizer": gpt2_original_tokenizer,
    ...
    "reverse_control": {
        "perturbation_function": partial(
            perturb_reverse_pair, rng=default_rng(21), reverse=False, full=False
        ),
        "affect_function": affect_reverse,
        "filter_function": filter_reverse,
        "gpt2_tokenizer": gpt2_rev_tokenizer,
    ...
    "reverse_partial": {
        "perturbation_function": partial(
            perturb_reverse_pair, rng=default_rng(21), reverse=True, full=False
        ),
        "affect_function": affect_reverse,
        "filter_function": filter_reverse,
        "gpt2_tokenizer": gpt2_rev_tokenizer,
    ...
    "reverse_full": {
        "perturbation_function": partial(
            perturb_reverse_pair, rng=default_rng(21), reverse=False, full=True
        ),
        "affect_function": affect_reverse,
        "filter_function": filter_reverse,
        "gpt2_tokenizer": gpt2_rev_tokenizer,
...
# Perturbations that are in focus
